...
vars ;;; PRB interface procedures 
     ...
     procedure dimensional_split_ew,
     procedure dimensional_split_ns,
     procedure de_equivalent,
...
uses prb_atms
uses new_LD_rules_EW_v1
uses new_LD_rules_NS_v2
;;; uses new_LD_rules
...
;;; 确保每一个relation都是同一个order，即 OS 在 SM 前,这里的BEQ其实没用，但写上去以防万一
;;; 初始的所有relation都是来自于统一数据库，alphabefore为1，只有BEQ产生的Nns和New才会有不同数据库的relation
...
define cannonicalise_datum(datum) -> cannonical_datum;
    ...
    if datum fmatches [?p ?a ?b] and alphabefore(b, a) then 
       if (p = "Iew" or p = "Ins" or p = "nEW" or p = "nNS" or p = "BEQ") then
           [^p ^b ^a]-> cannonical_datum; 
       elseif (p = "E") then
            [^("W") ^b ^a]-> cannonical_datum; 
       elseif (p = "W") then
            [^("E") ^b ^a]-> cannonical_datum; 
       elseif (p = "dE") then
            [^("dW") ^b ^a]-> cannonical_datum; 
       elseif (p = "dW") then
            [^("dE") ^b ^a]-> cannonical_datum; 
       elseif (p = "N") then
            [^("S") ^b ^a]-> cannonical_datum; 
       elseif (p = "S") then
            [^("N") ^b ^a]-> cannonical_datum; 
       elseif (p = "dN") then
            [^("dS") ^b ^a]-> cannonical_datum; 
       elseif (p = "dS") then
            [^("dN") ^b ^a]-> cannonical_datum; 
       else
            datum -> cannonical_datum;
       endif;
    else
...
;;; useless function, All directions are not equivalently repeated
;;; change all E to W, all N to S
define de_equivalent(relation);
    lvars p, a, b;

    if relation fmatches [?p ?a ?b] then 
       if (p = "E") then
            [^("W") ^b ^a]; 
       elseif (p = "dE") then
            [^("dW") ^b ^a]; 
...
;;; the ew relations return True
define dimensional_split_ew(relation) -> bool;
    lvars p, a, b;
...
;;;the ns relations return True
define dimensional_split_ns(relation) -> bool;
    lvars p, a, b;
    if relation fmatches [?p ?a ?b] and
    (p = "N" or p = "S" or p = "Ins" or p = "dN" or p = "dS" or p = "nNS") then
        true -> bool;
    else
        false -> bool;
    endif;
...
define beq_expand(relation);
    ...
    if relation fmatches [BEQ ?a ?b] then 
    	;;; [nEW ^a ^b], [nEW ^b ^a], [nNS ^a ^b], [nNS ^b ^a];
            [nEW ^a ^b], [nNS ^a ^b];
    else
	relation;
	endif;
...
;;; Check for inconsistent premises: a BPT/BEQ relation between objects
;;; in the same data set derives false.
define check_premises(premises);
    lvars premise;

    for premise in premises do
	if premise fmatches [BPT = =] or premise fmatches [BEQ = =] and not(mapping_relation(premise)) then 
...
;;; Only mapping relations can be assumptions -- other relations are premises.
define check_assumptions(assumptions);
    lvars assumption;

    for assumption in assumptions do
	if (assumption fmatches [E = =] or assumption fmatches [W = =] or assumption fmatches [Iew = =] or assumption fmatches [dE = =] or assumption fmatches [dW = =] or
        assumption fmatches [N = =] or assumption fmatches [S = =] or assumption fmatches [Ins = =] or assumption fmatches [dN = =] or assumption fmatches [dS = =]) 
        and not(mapping_relation(assumption)) then 
...
define ld_read(filename);
	...
	;;; 多边形的BPT的处理是一个问题（合并成BEQ？或者是改成NEW和nNS？）
    ;;; Expand the BPT, BEQ relations
	;;; 把所有的BPT换成BEQ （之后就没有BPT）
    maplist(ld_relations, beq_particle) -> ld_relations;

	;;; 把所有的BEQ换成nEW和nNS（之后就没有BEQ）
    ;;; print_nogood only print assumption in nogood!!!
    ;;; maplist(ld_relations, beq_expand) -> ld_relations;

    ;;; change to the corresponding equivalent relations;
    ;;; maplist(ld_relations, de_equivalent) -> ld_relations;
    ;;; ld_relations ==>

	;;; use cannonicalise_datum to order the rules
	;;; 检查是否Iew，Ins，nEW，nNS，BEQ都不会再由axioms生成：不会生成，但还是要换位置！
    ...
    ;;; printf('relation_set length = %p\n', [^(length(relation_set))]);

    ;;; Split into two dimensions
    lvars ew_relations, ns_relations;
    remove_if(dimensional_split_ew, relation_set) -> ns_relations;
    remove_if(dimensional_split_ns, relation_set) -> ew_relations;

    lvars total_nogood_number = 0, ns_nogoods = [], ew_nogoods = [];
...
;;; vars poparglist = ['nott_1.txt' 'nott_6.txt' 'nott_7.txt' 'nott_8.txt' 'nott_9.txt' 'nott_11.txt' 'nott_12.txt' 'nott_13.txt' 'nott_14.txt'];

;;; vars poparglist = ['nott_2.txt'];
...
;;;vars poparglist = ['south_7.txt' 'south_14.txt' 'south_22.txt'];
