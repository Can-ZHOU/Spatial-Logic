...
;;; These databases are used to record recurring relationships
vars procedure dN_db,
     procedure dS_db,
     procedure dE_db,
     procedure dW_db,
     procedure NS_AS2_db,
...
;;; Return true if some fact is not in database and add it into the database, otherwise return false (in the database) 
define some_not_in_db_p(datums,db)->bool;
lvars datum, bool=true;
	fast_for datum in datums do
		unless (not(datum_p(datum,db))) then
			false->bool;
			return;
...
;;;    atms_assume_all(premises);
    ...
    prb_newdatabase(64, []) -> dN_db;
    prb_newdatabase(64, []) -> dS_db;
    prb_newdatabase(64, []) -> dE_db;
    prb_newdatabase(64, []) -> dW_db;
    prb_newdatabase(64, []) -> NS_AS2_db;
    prb_newdatabase(64, []) -> NS_AS3_db;
    prb_newdatabase(64, []) -> NS_AS5_db;
    prb_newdatabase(64, []) -> EW_AS2_db;
    prb_newdatabase(64, []) -> EW_AS3_db;
    prb_newdatabase(64, []) -> EW_AS5_db;
