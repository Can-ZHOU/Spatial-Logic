...
;;;	[DLOCAL [prb_show_conditions = true]];
	[VARS prb_allrules trigger_db dN_db dS_db NS_AS2_db NS_AS3_db NS_AS5_db];
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	[dN ?A ?B] [->> a1]
    [Ins ?A ?B] [->> a2]
    [WHERE some_not_in_db_p([^a1], dN_db)]
    [WHERE some_not_in_db_p([^a2], dN_db)]
	[WHERE some_in_db_p([^a1 ^a2], trigger_db)]
	[LVARS [consequent = add_new_formula([N ^A ^B])]]
    [POP11 prb_add_to_db([dN ^A ^B],dN_db); prb_add_to_db([Ins ^A ^B],dN_db); prb_add_to_db([N ^A ^B],dN_db)]
    ==>
    ...
    [POP11 prb_add_to_db([N ^A ^B],dN_db); prb_add_to_db([dN ^A ^B],dN_db); prb_add_to_db([Ins ^A ^B],dN_db)]
    ==>
...
*/
	...
	[dS ?A ?B] [->> a1]
    [Ins ?A ?B] [->> a2]
    [WHERE some_not_in_db_p([^a1], dS_db)]
    [WHERE some_not_in_db_p([^a2], dS_db)]
	[WHERE some_in_db_p([^a1 ^a2], trigger_db)]
	[LVARS [consequent = add_new_formula([S ^A ^B])]]
    [POP11 prb_add_to_db([dS ^A ^B],dS_db); prb_add_to_db([Ins ^A ^B],dS_db); prb_add_to_db([S ^A ^B],dS_db)]
    ==>
	...
	[LVARS [consequent1 = add_new_formula([dS ^A ^B])]
	       [consequent2 = add_new_formula([Ins ^A ^B])]]
    [POP11 prb_add_to_db([S ^A ^B],dS_db); prb_add_to_db([dS ^A ^B],dS_db); prb_add_to_db([Ins ^A ^B],dS_db)]
    ==>
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	RULE NS_axiom_2_1
	[N ?A ?B] [->> a1]
    [WHERE some_not_in_db_p([^a1], NS_AS2_db)]
	[WHERE some_in_db_p([^a1], trigger_db)]
    [LVARS [consequent = add_new_formula([S ^B ^A])]]
    [POP11 prb_add_to_db([N ^A ^B],NS_AS2_db); prb_add_to_db([S ^B ^A],NS_AS2_db)]
    ==>
	...
	[WHERE some_in_db_p([^a1], trigger_db)]
    [LVARS [consequent = add_new_formula([N ^A ^B])]]
    [POP11 prb_add_to_db([S ?B ?A],NS_AS2_db); prb_add_to_db([N ^A ^B],NS_AS2_db)]
    ==>
	...
	RULE NS_axiom_3
	[Ins ?A ?B] [->> a1]
    [WHERE some_not_in_db_p([^a1], NS_AS3_db)]
	[WHERE some_in_db_p([^a1], trigger_db)]
    [LVARS [consequent = add_new_formula([Ins ^B ^A])]]
    [POP11 prb_add_to_db([Ins ?A ?B],NS_AS3_db); prb_add_to_db([Ins ^B ^A],NS_AS3_db)]
    ==>
	...
	RULE NS_axiom_5_1
	[dN ?A ?B] [->> a1]
    [WHERE some_not_in_db_p([^a1], NS_AS5_db)]
	[WHERE some_in_db_p([^a1], trigger_db)]
    [LVARS [consequent = add_new_formula([dS ^B ^A])]]
    [POP11 prb_add_to_db([dN ?A ?B],NS_AS5_db); prb_add_to_db([dS ^B ^A],NS_AS5_db)]
    ==>
    ...
    [POP11 prb_add_to_db([dS ?A ?B],NS_AS5_db); prb_add_to_db([dN ^B ^A],NS_AS5_db)]
    ==>
