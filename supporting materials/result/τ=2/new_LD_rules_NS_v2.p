...
;;; N/S/dN/dS/BEQ/Ins needs converse
...
;;; a表示是第几个变体公式，a前面的数字是第几个axioms，a后面的数字是此变体公式的第几次转换order
	...
	;;; AS 8 dS(a,b) ∧ ¬N(b,c) → S(a,c)
	;;; dS(a,b) ∧ BEQ(b,c) → S(a,c) or dS(a,b) ∧ S(b,c) → S(a,c)
	RULE NS_axiom_8a_1
	[dS ?A ?B] [->> a1]
	[BEQ ?B ?C] [->> a2]
	[WHERE some_in_db_p([^a1 ^a2], trigger_db) and alphabefore(A, C)]
	...
	;;; AS 9 ¬N(a,b) ∧ dS(b,c) → S(a,c)
	;;; BEQ(a,b) ∧ dS(b,c) → S(a,c) or S(a,b) ∧ dS(b,c) → S(a,c)
	
	;;; AS 10 S(a,b) ∧ ¬N(b,c) ∧ R(c,d) -> R(a,d), where R∈｛S,dS,¬N,¬dN｝
	;;; AS 10_1a:  S(a,b) ∧ BEQ(b,c) ∧ S(c,d) -> S(a,d) (16)
	...
	;;; 10_2a: S(a,b) ∧ BEQ(b,c) ∧ dS(c,d) → dS(a,d),delete
	...
	;;; AS 10.3a: dS(d,b) ∧ BEQ(b,c) -> N(c,d) delete
	;;; AS 10.3b: S(d,a) ∧ S(a,c)  ->  S(d,c) delete
	
	;;; As 10.4a: S(a,b) ∧ BEQ(b,c) ∧ dN(a,d) -> dN(c,d)
	...
	;;; As 11.1a: dS(b,d) ∧ BEQ(a,b) -> S(a,d) delete
	;;; AS 11.1b: repeat

	;;; AS 11.2a: BEQ(a,b) ∧ dS(b,d) -> S(a,d) delete
	;;; AS 11.2b: repeat

	;;; AS 11.3a: S(b,c) ∧ BEQ(a,b) ∧ N(a,d) -> N(c,d)
