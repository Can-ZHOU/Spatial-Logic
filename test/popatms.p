...
;;;	       [ ^(length(datalist(atms_assumption_table)))
    ...
    printf('\nATMS nogoods\n\n');
    lvars i, nogood_count=0;
	;;;'atms_nogoods is like: ' => 
	;;;datalist(atms_nogoods)==>
    printf('extensible_vector_length-1 is %p\n',[^(extensible_vector_length(atms_nogoods) - 1)]);
    ...
    print_extensible_vector(atms_nogoods);

    ;;; Print information about environments
    ;;; datalist(atms_environments)==>
    printf('\nATMS environments\n\n');
    lvars environment_count=0;
    ...
    lvars cseq_count=0, longest_cseq_count=0;
